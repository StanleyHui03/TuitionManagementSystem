...
use App\Http\Controllers\TutorLessonConsumerController;
...
Route::middleware(['auth', 'notBanned'])->group(function () {
    ...
    Route::get('/tutor/dashboard', [TutorLessonController::class, 'dashboard'])
        ->name('tutor.dashboard');

    Route::get(
        '/tutor/api-lessons',
        [TutorLessonConsumerController::class, 'index']
    )->name('tutor.apiLessons');

    // Attendance -> tutor lessons 列表页
    Route::get('/tutor/lessons', [TutorLessonController::class, 'index'])
        ->name('tutor.lessons');

    // Material -> 直接渲染视图 resources/views/materials/material.blade.php
    Route::view('/tutor/materials', 'materials.material')
...
Route::middleware(['auth'])->group(function () {
    ...
    // JSON viewer page (only tutor-owner/admin can see inside controller)
    Route::get(
        '/lessons/{lesson:lesson_id}/attendance/json-viewer',
        [AttendanceController::class, 'jsonViewer']
    )
        ->name('attendance.json.viewer');
...
Route::middleware(['auth'])->prefix('api/v1')->group(function () {
    Route::get('/lessons/{lesson:lesson_id}/attendance', [AttendanceController::class, 'apiLessonAttendance']);
    Route::post('/lessons/{lesson:lesson_id}/attendance', [AttendanceController::class, 'apiUpdateAttendance']);
    Route::get('/students/me/attendance', [AttendanceController::class, 'apiStudentAttendance']);
});
